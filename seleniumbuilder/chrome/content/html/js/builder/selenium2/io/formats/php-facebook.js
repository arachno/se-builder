builder.selenium2.io.addLangFormatter({
  ...
  "require_once 'lib/__init__.php';" +
	...
	"WebDriverCapabilityType::TAKES_SCREENSHOT => true\n"+
	");\n"+
  ...
  lineForType: {
    ...
    "refresh":
      "$driver->refresh();\n",
    "clickElement":
      "$driver->findElement(WebDriverBy::id('{locator}'))->click();\n",
    "mouseOverElement":
      "$session->moveto(array(\"element\" => $session->element({locatorBy}, {locator})->getID()));\n",
    "setElementText":
      "$session->findElement(WebDriverBy::id('{locator}'))->sendkeys({text});\n",
    "sendKeysToElement":
